...
app.post('/doInsert',async (req,res)=>{
    ...
    let datetimeInput = req.body.txtDatetime;
    let client= await MongoClient.connect(url);  
    let dbo = client.db("ASM2"); 
    let newProduct = {productName:nameInput,price:priceInput,color:colorInput,datetime:datetimeInput};
...
app.post('/doEdit',async (req,res)=>{
    ...
    let colorInput = req.body.txtColor;
    let datetimeInput = req.body.txtDatetime;
    let newValues ={$set : {productName:nameInput,price:priceInput,color:colorInput,datetime:datetimeInput}};
    var ObjectID = require('mongodb').ObjectID;
    let condition = {"_id" : ObjectID(id)};
